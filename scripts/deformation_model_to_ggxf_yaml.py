...
def main():
    ...
    parser.add_argument(
        "--discard-temporal-extent",
        action="store_true",
        help="Don't included temporal extent in YAML",
    )
    parser.add_argument(
        "-m",
        "--metadata-yaml",
    ...
    gmodel = ggxfModel(
        smodel,
        usegroups=args.group,
        maxdepth=args.depth,
        maxwidth=args.width,
        notemporalextent=args.discard_temporal_extent,
    )
...
def ggxfModel(
    model, usegroups=None, maxwidth=None, maxdepth=None, notemporalextent=False
):
    gmodel = {}
    ...
    if not notemporalextent:
        gmodel["contentApplicabilityExtent"]["extentTemporal"] = {
